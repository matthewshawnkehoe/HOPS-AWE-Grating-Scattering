function [xi_r_n_m,nu_r_n_m] = setup_xi_w_nu_w_n_m(B,r,xx,pp,...
    alpha_bar_p,gamma_bar_p,f,f_x,Nx,N,M)
% setup_xi_w_nu_w_n_m.m: Setup variables for calculating the approximate 
% solution (xi_r_n_m) and derivative of the approximate solution (nu_r_n_m)
% in the lower layer for validating the two layer solver.
%
%  Inputs:
%   B: Coefficients of the Helmholtz equation in the lower layer
...
%
%  Outputs:
%   xi_r_n_m: tensor representing the approximate solution in the lower 
...
%             in the lower layer at the surface z=g(x) for a particular 
%             wave number p
  ...
  for m=0:M
    ...
    for ell=0:m
      nu_r_n_m(:,m+1,n+1) = nu_r_n_m(:,m+1,n+1)...
          - lower.*(1i*gamma_r_m(m-ell+1)).*E_n_m(:,ell+1,n+1);
    end
    if(n>0)
      nu_r_n_m(:,m+1,n+1) = nu_r_n_m(:,m+1,n+1)...
          - lower.*f_x.*(1i*pp_r).*E_n_m(:,m+1,n-1+1);
    end    
  end
end
