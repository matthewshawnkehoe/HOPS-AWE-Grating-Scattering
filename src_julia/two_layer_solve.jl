...
function two_layer_solve(tau2,zeta_n_m,psi_n_m,
  ...
  Q = zeta_n_m[:,0+1,0+1]
  R = -psi_n_m[:,0+1,0+1]
  U_n_m[:,0+1,0+1],W_n_m[:,0+1,0+1] = AInverse(Q,R,gamma_u_bar_p,gamma_w_bar_p,Nx,tau2)
  ...
  for n=0:N
    for m=0:M
      ...
      if n > 1 && m > 0
        R = R - f_x*1im*alpha_bar.*U_n_m[:,m,n-1] + tau2*f_x*1im*alpha_bar.*W_n_m[:,m,n-1]
        if m > 1
          R = R - f_x*1im*alpha_bar.*U_n_m[:,m-1,n-1] + tau2*f_x*1im*alpha_bar.*W_n_m[:,m-1,n-1]
        end
      end
      for r=0:n
        for s=0:m
          if((r<n)||(s<m))
            xi_n_m = zeros(Complex{Float64},Nx,M+1,N+1)
            ...
            G_n_m = dno_tfe_helmholtz_m_and_n(u_n_m,f,pp,Dz,a,Nx,Nz,n-r,m-s)
            ...
            J_n_m = dno_tfe_helmholtz_m_and_n_lf(w_n_m,f,pp,Dz,b,Nx,Nz,n-r,m-s)
