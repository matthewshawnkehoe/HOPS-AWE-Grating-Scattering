...
# MSK 06/18/21 Created for testing the Method of Manufactured Solutions
...
for s in eachindex(qq)
  ...
  xi_u_r_n_m,nu_u_r_n_m = setup_xi_u_nu_u_n_m(A,r,xx,pp,alpha_bar_p,
                    gamma_u_bar_p,f,f_x,Nx,N,M)
  ...
  U_n_m,W_n_m,ubar_n_m,wbar_n_m = two_layer_solve_fast(tau2,zeta_r_n_m,
    psi_r_n_m,gamma_u_bar_p,gamma_w_bar_p,N,Nx,f,f_x,pp,alpha_bar,gamma_u_bar,
  ...
  for j=1:N_Eps
    for ell=1:N_delta
      ...
      # Exact
      xi_u_r = @. A*exp(1im*pp_r*xx)*exp(1im*gamma_u_r*Eps[j]*f)
      nu_u_r = @. (-1im*gamma_u_r + 1im*pp_r*Eps[j]*f_x)*xi_u_r
      xi_w_r = @. B*exp(1im*pp_r*xx)*exp(-1im*gamma_w_r*Eps[j]*f)
      # Add a negative sign from taking -\partial_N w
      nu_w_r = @. (-1im*gamma_w_r - 1im*pp_r*Eps[j]*f_x)*xi_w_r
      ubar = @. A*exp(1im*pp_r*xx)*exp(1im*gamma_u_r*a)
      wbar = @. B*exp(1im*pp_r*xx)*exp(-1im*gamma_w_r*(-b))
  ...
  # Plot the MMS error
  fig1 = plot_mms_error(omega, Eps, err_U_pade)
  fig2 = plot_mms_error(omega, Eps, err_ubar_pade)
  display(fig1)
  display(fig2)
end