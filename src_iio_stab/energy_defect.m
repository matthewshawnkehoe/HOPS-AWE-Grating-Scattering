...
for ell=1:N_delta
  for j=1:N_Eps
    ...
    [vbar] = fcn_sum_fast(SumType,vbar_n_m,Eps(j),delta(ell),Nx,N,M);
    ...
    C = gamma_v_p./gamma_i.*abs(vbarhat).^2;
    D = gamma_w_p./gamma_i.*abs(wbarhat).^2;
    ...
    rv(j,ell) = sum(PropMode_v.*C);         % What should this be?
